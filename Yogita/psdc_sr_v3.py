...
class SRGenerator(nn.Module):
    def __init__(self, feature_channels, latent_channels=32, num_res=8, scale=4, out_channels=3):
        ...
        self.up_blocks   = nn.Sequential(*[UpBlock(feature_channels) for _ in range(scale // 2)])
...
# ===========================================================================
# 12.  -Cross Ablation Subclass
# ===========================================================================

class NoCrossModel(PSDCSRModel):
    """
    Ablation variant: '-Cross'
    Removes cross-conditioning by using each LR's OWN degradation code,
    i.e. sr1 = G(LR1, z1) and sr2 = G(LR2, z2) instead of swapping.

    This is the single most important ablation — the PSNR/SSIM gap between
    this and the full model is your proof that cross-conditioning helps.
    Everything else (losses, weights, encoder, generator) is identical.
    """
    ...
    def forward(self, lr1, lr2, hr):
        ...
        # ── NO cross: each LR uses its own code ─────────────────────────────
        sr1     = self.generator(f1, z1)   # was G(f1, z2) in full model
        sr2     = self.generator(f2, z2)   # was G(f2, z1) in full model
        sr_self = self.generator(f1, z1)

        # ── Cycle on both outputs ────────────────────────────────────────────
        lr_recon1 = self.deg_net(sr1)
        lr_recon2 = self.deg_net(sr2)
        ...
        # ── Losses (identical to parent) ─────────────────────────────────────
        l_rec    = F.l1_loss(sr1, hr) + F.l1_loss(sr2, hr)
        l_self   = F.l1_loss(sr_self, hr)
        l_kl     = kl_consistency(mu1, logvar1, mu2, logvar2)
        l_cycle  = F.l1_loss(lr_recon1, lr1_m) + F.l1_loss(lr_recon2, lr2_m)
        l_smooth = spatial_smoothness(z1) + spatial_smoothness(z2)
        l_var    = variance_regularization(logvar1, logvar2)
        l_div    = diversity_loss(z1, z2)
        l_fft    = fft_loss(sr1, hr) + fft_loss(sr2, hr)
...
def build_ablation_variant(name: str, **kwargs) -> PSDCSRModel:
    ...
    name options:
        ...
        '-Cross'  — no cross-conditioning; uses NoCrossModel subclass
    ...
    Usage:
        ...
        model_nocross = build_ablation_variant('-Cross')
    """
    ...
    all_names = list(overrides.keys()) + ['-Cross']
    if name not in all_names:
        raise ValueError(f'Unknown variant "{name}". Choose from: {all_names}')

    defaults.update(overrides.get(name, {}))

    # -Cross uses the subclass that disables code swapping in forward()
    if name == '-Cross':
        return NoCrossModel(**defaults)
...
if __name__ == '__main__':
    ...
    # Ablation variants — verify all build and produce correct output shapes
    print('\nAblation variant check:')
    for name in ['full', '-KL', '-Cross', '-Cycle', '-Var', '-Div', '-FFT']:
        m   = build_ablation_variant(name).to(device)
        n   = sum(p.numel() for p in m.parameters() if p.requires_grad)
        o   = m(lr1, lr2, hr)
        tag = 'NoCrossModel' if isinstance(m, NoCrossModel) else 'PSDCSRModel'
        print(f'  {name:8s} [{tag}]  params: {n:,}  SR1: {o["sr1"].shape}')
